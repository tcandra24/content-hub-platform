...
const { data: d } = await useFetch(`/api/admin/articles/${id}`);
...
const { handleSubmit, errors, defineField, isSubmitting } = useForm({
  ...
  initialValues: {
    title: d.value?.article?.title!,
    slug: d.value?.article?.slug!,
    excerpt: d.value?.article?.excerpt!,
    content: d.value?.article?.content!,
    categoryId: d.value?.article?.categoryId!,
    contentType: d.value?.article?.contentType!,
    seoTitle: d.value?.article?.seoTitle!,
    seoDescription: d.value?.article?.seoDescription!,
    readingTime: d.value?.article?.readingTime!,
    tags: d.value?.article?.articleTags?.map((tag) => tag.tag.name) || [],
    status: d.value?.article?.status!,
  },
});
